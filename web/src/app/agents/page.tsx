    ...
    <div className="mx-auto max-w-[1400px] px-6 lg:px-10 py-8">
      ...
      {stats && (
        <div className="flex flex-wrap items-center gap-x-8 gap-y-2 text-xs mb-8">
          <span><span className="text-[#a1a1aa]/60">Scanned </span><span className="font-mono tabular-nums text-[#fafafa]">{stats.totalScanned}</span></span>
          <span><span className="text-[#a1a1aa]/60">Safe </span><span className="font-mono tabular-nums text-[#34d399]">{stats.totalSafe}</span></span>
          <span><span className="text-[#a1a1aa]/60">Caution </span><span className="font-mono tabular-nums text-[#f59e0b]">{stats.totalCaution}</span></span>
          <span><span className="text-[#a1a1aa]/60">Dangerous </span><span className="font-mono tabular-nums text-[#ef4444]">{stats.totalDangerous}</span></span>
          <span><span className="text-[#a1a1aa]/60">Events </span><span className="font-mono tabular-nums text-[#fafafa]">{eventStats?.totalEvents ?? 0}</span></span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-10">
        {agents.map((agent) => {
          ...
          return (
            <div
              key={agent.id}
              className="rounded-md border border-[#27272a]/50 p-5 flex flex-col"
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] text-[#a1a1aa]/40 ml-auto">{cfg.role}</span>
              </div>

              {/* Description */}
              <p className="text-[11px] text-[#a1a1aa]/50 mb-4 leading-relaxed">
                {cfg.description}
              </p>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <a
                    ...
                    className="text-[11px] font-mono text-[#a1a1aa]/50 hover:text-[#8b5cf6] transition-colors flex items-center gap-1"
                  >
                ...
                <div className="flex items-center gap-4 text-xs">
                  <span>
                    <span className="text-[#a1a1aa]/40">Balance </span>
                    <span className="font-mono tabular-nums text-[#fafafa]">{Number(agent.usdtBalance).toFixed(2)} USDT</span>
                  </span>
                  <span>
                    <span className="text-[#a1a1aa]/40">Events </span>
                    <span className="font-mono tabular-nums text-[#fafafa]">{agentEventCount}</span>
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-1 mb-4">
                {cfg.skills.map((skill) => (
                  <span key={skill} className="rounded px-1.5 py-px text-[9px] text-[#a1a1aa]/40 bg-[#27272a]/40">
                    {skill}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-auto">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.12em] text-[#a1a1aa]/40">Recent</span>
                </div>
                {agentEvents.length === 0 ? (
                  <p className="text-[10px] text-[#a1a1aa]/30">No events yet</p>
                ) : (
                  <div className="space-y-1">
                    {agentEvents.map((evt, i) => (
                      <div key={`${evt.timestamp}-${i}`} className="flex items-start gap-2">
                        ...
                        <span className="text-[10px] text-[#a1a1aa]/50 leading-tight line-clamp-1">{evt.message}</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {eventStats && Object.keys(eventStats.byType).length > 0 && (
        <div>
          <h2 className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[#a1a1aa] mb-3">
            Event Breakdown
          </h2>
          <div className="flex flex-wrap gap-3">
            {Object.entries(eventStats.byType)
              .sort(([, a], [, b]) => b - a)
              .map(([type, count]) => (
                <div key={type} className="flex items-center gap-2 rounded border border-[#27272a]/30 px-3 py-1.5">
                  <span className="text-[10px] uppercase tracking-wider text-[#a1a1aa]/50">{type}</span>
                  <span className="text-xs font-mono tabular-nums text-[#fafafa]">{count}</span>
                </div>
              ))}
          </div>
        </div>
